...
export const Route = createFileRoute("/_authenticated/onboarding")({
  head: () => ({ meta: [{ title: "Conectar dispositivo — Hannya" }] }),
...
function Onboarding() {
  ...
  if (step === "success") {
    return (
      <div className="min-h-screen px-6 py-10 flex flex-col items-center justify-center text-center bg-background">
        <div className="size-24 rounded-full bg-primary/15 grid place-items-center animate-in zoom-in">
          <CheckCircle2 className="size-12 text-primary" />
        </div>
        <h1 className="mt-6 text-2xl font-bold">Dispositivo conectado!</h1>
        <p className="mt-2 text-sm text-muted-foreground">Você já está protegida. Levando você ao Hannya…</p>
      </div>
    );
  }
  ...
  return (
    <div className="min-h-screen px-6 py-10 flex flex-col">
      ...
      <p className="text-sm text-muted-foreground mb-6">
        Insira o código único de rastreio que veio na caixa do seu kit Hannya. Ele aparece na embalagem do chaveiro rastreador e do botão SOS.
      </p>

      <Card className="p-4 rounded-2xl border-border/60 mb-6">
        <p className="text-xs font-medium text-muted-foreground mb-3">Dispositivos vinculados ao app</p>
        ...
        <p className="mt-3 text-[11px] text-muted-foreground text-center">
          O gloss faz parte do kit, mas não precisa ser pareado.
        </p>
      </Card>
